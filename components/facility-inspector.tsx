...
export function FacilityInspector({ facility, data, onClose }: { facility: Facility; data: AtlasDataset; onClose(): void }) {
  ...
  const sourceIds = new Set([
    ...
    ...(facility.annualGeneration?.sourceObservationIds.flatMap((id) => observations.get(id)?.sourceId ?? []) ?? []),
  ...
  const generation = facility.annualGeneration ? `${facility.annualGeneration.value.toLocaleString()} ${facility.annualGeneration.unit} (${facility.annualGeneration.year}, final)` : "Not reported";
  const dateLabel = facility.capacities.some((item) => item.status === "installed") ? "Operating since" : "Target operation (preliminary)";
  return <aside className="inspector" aria-label="Facility inspector">
    ...
    <section><h3>Physical record</h3><dl><dt>Operator</dt><dd>{operators.join(", ") || "Not reported"}</dd><dt>Technology</dt><dd>{facility.technologyLabel}</dd><dt>Capacity</dt><dd>{capacity}</dd><dt>Final generation</dt><dd>{generation}</dd><dt>{dateLabel}</dt><dd>{facility.stateDate ? new Date(`${facility.stateDate}T00:00:00Z`).toLocaleDateString() : "Not reported"}</dd><dt>Energy role</dt><dd>{title(facility.energyRole)}</dd><dt>Project ID</dt><dd><code>{facility.projectId}</code></dd><dt>Phases</dt><dd>{facility.phaseIds.length ? `${facility.phaseIds.length} generator phase${facility.phaseIds.length === 1 ? "" : "s"}` : "No phase published"}</dd></dl>{facility.phaseIds.length > 0 && <details><summary>Stable phase identifiers</summary><ul>{facility.phaseIds.map((id) => <li key={id}><code>{id}</code></li>)}</ul></details>}</section>
